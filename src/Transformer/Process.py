...
def read_data(opt):
    ...
    if opt.src_data is not None:
        try:
            ...
            #opt.src_data = np.load(os.path.join(opt.src_data))
        except:
            print("error: '" + opt.src_data + "' file not found")
            quit()
    
    if opt.trg_data is not None:
        opt.trg_data = open(opt.trg_data, encoding='latin1').read().strip().split('\n')
        #opt.trg_data = np.load(os.path.join(opt.trg_data))
