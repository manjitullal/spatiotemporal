...
def compute_integral_unbiased(model, data, time, latitude, longitude, non_pad_mask, type_mask):
    ...
    diff_latitude = (latitude[:, 1:] - latitude[:, :-1]) * non_pad_mask[:, 1:]
    temp_latitude = diff_latitude.unsqueeze(2) * torch.rand([*diff_latitude.size(), num_samples], device=data.device)
    temp_latitude /= (latitude[:, :-1] + 1).unsqueeze(2)

    diff_longitude = (longitude[:, 1:] - longitude[:, :-1]) * non_pad_mask[:, 1:]
    temp_longitude = diff_longitude.unsqueeze(2) * torch.rand([*diff_longitude.size(), num_samples], device=data.device)
    temp_longitude /= (longitude[:, :-1] + 1).unsqueeze(2)

    temp_hid = model.linear(data)[:, 1:, :]
    temp_hid = torch.sum(temp_hid * type_mask[:, 1:, :], dim=2, keepdim=True)

    all_lambda = F.softplus(temp_hid + model.alpha * temp_time + model.alpha * temp_latitude + model.alpha * temp_longitude, threshold=10)
    all_lambda = torch.sum(all_lambda, dim=2) / num_samples

    unbiased_integral = all_lambda * diff_time*diff_latitude*diff_longitude
    return unbiased_integral


def log_likelihood(model, data, time, types, latitude, longitude):
    ...
    non_event_ll = compute_integral_unbiased(model, data, time, latitude, longitude, non_pad_mask, type_mask)
